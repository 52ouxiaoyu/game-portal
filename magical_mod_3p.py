...
class Game {
    ...
    startGame() {
        ...
        for (let i = 1; i <= this.numPlayers; i++) {
            const config = heroConfigs[i - 1];
            this.heroes.push({
                x: (CONFIG.CANVAS_WIDTH / (this.numPlayers + 1)) * i,
                y: CONFIG.HERO.y,
                color: config.color,
                name: config.name,
    ...
    update(currentTime, deltaTime) {
        ...
        this.heroes.forEach(hero => {
            if (this.keys[hero.controls[0]] || this.keys[hero.controls[1]]) {
                hero.x -= CONFIG.HERO.speed;
            }
            if (this.keys[hero.controls[2]] || this.keys[hero.controls[3]]) {
                hero.x += CONFIG.HERO.speed;
            }
            hero.x = Math.max(20, Math.min(CONFIG.CANVAS_WIDTH - 20, hero.x));
...
print("3 Players and dynamic lanes successfully added!")
